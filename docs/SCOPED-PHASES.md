...
Only after that review may migration 004 be applied and held enrichment begin using the explicit staged backbone publication ID as the third OpenF1 argument. Use mapped representative sessions, measure database/raw-cache growth, then bounded batches. OpenF1 historical coverage begins in 2023; absent earlier coverage remains unavailable. Preserve Jolpica laps/pits where supplied. F1DB uses the checksum-pinned release and reviewed canonical identity mapping through the namespaced held-overlay importer; it never writes the public snapshot or the base aliases. Never use the old direct-publication provider path for archive-wide detail updates.
