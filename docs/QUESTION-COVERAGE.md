...
Weather, tyre stints, race control, telemetry, radio, overtakes and historical scoring are only answerable for sessions where their normalized datasets are published. They should be added as separate intents after their field definitions and coverage checks are complete; retaining a provider payload alone is not enough to expose a trustworthy answer.
