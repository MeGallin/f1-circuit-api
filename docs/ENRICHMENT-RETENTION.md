...
Las Vegas 2024 (OpenF1 key 9644) uses a reviewed, event-scoped UTC rollover mapping. It requires the canonical session/event identity, round 22, `circuit:vegas`, OpenF1's `Las Vegas` circuit, and the `America/Los_Angeles` local date to agree before accepting the one-day UTC difference. Unrelated one-day mismatches continue to fail closed. No activation, deployment, snapshot pruning or public-pointer switch occurred.
