...
1. Deterministic templates handle safe, recognisable query shapes without an AI provider. This includes archive searches, event winners, circuit race-winner history, podiums, qualifying pole, fastest laps, pit-stop counts and leaders, driver wins and comparisons, second-place or runner-up leader counts, driver history, driver/constructor counts, separate driver metrics for starts, podiums, poles, fastest laps, retirements, disqualifications, DNS and DNQ, plus published session metrics for weather, tyre strategy, race control and overtakes.
...
Second-place leader questions count published race-result rows with position `2`, grouped by their canonical driver ID. The answer is limited to the published archive, preserves ties, includes result evidence IDs, and reports unavailable when race positions or driver linkage are not published. Partial source coverage is exposed in the structured values instead of being presented as a complete historical total.
